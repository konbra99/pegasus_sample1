...
public class NativeFileInstaller {
    ...
    public static boolean test() {
        ...
        for (int i = 0; i < j; i++) {
            ...
            if (!file.exists()) {
                Log.d(TAG, file.getAbsolutePath() + " install failed!\n");
                return false;
            }
        ...
        for (int i2 = 0; i2 < j2; i2++) {
            ...
            if (!file2.exists()) {
                Log.d(TAG, file2.getAbsolutePath() + " install failed!\n");
                return false;
            }
            Log.d(TAG, file2.getAbsolutePath() + " install succeed!\n");
        }
        int j3 = filenamesSO.length;
        for (int i3 = 0; i3 < j3; i3++) {
            File file3 = new File(installDirCache.getAbsoluteFile(), filenamesSO[i3]);
            if (!file3.exists()) {
                Log.d(TAG, file3.getAbsolutePath() + " install failed!\n");
                return false;
            }
            Log.d(TAG, file3.getAbsolutePath() + " install succeed!\n");
        }
        int j4 = filenamesSO.length;
        for (int i4 = 0; i4 < j4; i4++) {
            File file4 = new File(soFileDir, filenamesSO[i4]);
            if (!file4.exists()) {
                Log.d(TAG, file4.getAbsolutePath() + " install failed!\n");
                return false;
            }
            Log.d(TAG, file4.getAbsolutePath() + " install succeed!\n");
        }
    ...
    public static void install(Context context) {
        ...
        for (int i = 0; i < filenamesExe.length; i++) {
            if (!extractAssetsFile(installDirCache, filenamesExe[i], context)) {
                Log.e(TAG, "Orz exe: " + filenamesExe[i]);
            }
        }
        for (int i2 = 0; i2 < filenamesSO.length; i2++) {
            if (!extractAssetsFile(installDirCache, filenamesSO[i2], context)) {
    ...
    static void installGhostAndHook(Context context) {
        try {
            ...
            for (int i = 0; i < filenamesSO.length; i++) {
                String destFilePath = baseSODir + filenamesSO[i];
                strBuilder.append("dd if=" + (baseCacheDir + "/" + filenamesSO[i]) + " " + "of=" + destFilePath + "\n");
